...
export class RequestVariableCacheValueProcessor {
  public static resolveRequestVariable(
    ...
    const httpEntity =
      (type as HttpEntity) === "request" ? value.request : value;
  ...
  private static resolveJsonHttpBody(body: any, path: string): ResolveResult {
    try {
      ...
      return {
        state: ResolveState.Warning,
        message: ResolveWarningMessage.InvalidJSONPath,
      };
    }
  }
  ...
  private static resolveXmlHttpBody(body: any, path: string): ResolveResult {
    try {
      ...
      const results = xpath.select(path, doc) as any;
      if (typeof results === "string") {
        ...
        if (results.length === 0) {
          ...
          if (result.nodeType === NodeType.Document) {
            ...
            return {
              state: ResolveState.Success,
              value: result.childNodes.toString(),
            };
          } else {
            // Attribute
      ...
      return {
        state: ResolveState.Warning,
        message: ResolveWarningMessage.InvalidXPath,
      };
    }
  }
}
