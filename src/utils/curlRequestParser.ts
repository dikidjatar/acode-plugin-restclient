...
export class CurlRequestParser implements RequestParser {
  ...
  public async parseHttpRequest(name?: string): Promise<HttpRequest> {
    ...
    if (typeof body === "string" && body[0] === "@") {
      ...
      if (fileAbsolutePath) {
        body = await fs(fileAbsolutePath).readFile("utf-8");
      } else {
    ...
    // parse method
    let method: string = (parsedArguments.X ||
      parsedArguments.request) as string;
    if (!method) {
      method = body ? "POST" : "GET";
    }

    return new HttpRequest(method, url as string, headers, body, name);
  }
